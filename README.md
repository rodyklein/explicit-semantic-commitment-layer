...
A minimal open-source layer enabling users to explicitly declare the meaning they assign to terms, and requiring AI systems to treat these declarations as persistent semantic commitments.

This repository implements a missing infrastructural capability for human-centered AI systems: the recognition of explicit user-defined meaning as a procedural normative constraint, rather than as transient conversational context.

This layer is intentionally procedural and agnostic with respect to semantic correctness. It does not replace ontologies, formal semantic systems, or statistical language models, but constrains their operation by explicitly declared user meaning.
...
Contemporary AI systems, including large language models (LLMs), are highly effective at adapting to conversational context. They can temporarily follow a user’s vocabulary, framing, or perspective during an interaction.
...
The issue is not model performance, but the absence of an explicit semantic commitment layer.
...
procedural priority of declared meaning over inferred meaning,

revision authority restricted to the declaring user,

comparison of semantic commitments between users without forced resolution.
...
Explicit semantic commitments take procedural precedence over inferred interpretations within the scope of interaction.
...
treat it as procedurally normative,
...
procedural semantic comparison.
...
Ethical and philosophical implications of semantic authority and user autonomy are discussed in the associated Zenodo publication.
...
Integration requires only that the host system consult the commitment registry before generating or interpreting language.
...
If you use this work in academic research, please cite it using the provided CITATION.cff file and the associated Zenodo DOI.
